...
// static
std::string SysInfo::OperatingSystemName() { return "Mac OS X"; }

// static
std::string SysInfo::OperatingSystemVersion() {
  int32_t major, minor, bugfix;
  OperatingSystemVersionNumbers(&major, &minor, &bugfix);
  return absl::StrFormat("%d.%d.%d", major, minor, bugfix);
}

// static
void SysInfo::OperatingSystemVersionNumbers(int32_t* major_version, int32_t* minor_version,
...
int64_t SysInfo::AmountOfPhysicalMemoryImpl() {
  ...
  int result =
      host_info(host.get(), HOST_BASIC_INFO, reinterpret_cast<host_info_t>(&hostinfo), &count);
...
// static
std::string SysInfo::HardwareModelName() { return GetSysctlValue("hw.model"); }

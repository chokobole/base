...
struct NumberOfProcessorsGetter {
  NumberOfProcessorsGetter() { Init(); }

  void Init() {
    // sysconf returns the number of "logical" (not "physical") processors on
    // both
    // Mac and Linux.  So we get the number of max available "logical"
    // processors.
    //
...
struct AmountOfVirtualMemoryGetter {
  AmountOfVirtualMemoryGetter() { Init(); }

  void Init() {
    struct rlimit limit;
    CHECK_NE(getrlimit(RLIMIT_DATA, &limit), 0);
    value = limit.rlim_cur == RLIM_INFINITY ? 0 : limit.rlim_cur;
  }
...
bool IsStatsZeroIfUnlimited(const base::FilePath& path) {
  ...
  if (HANDLE_EINTR(statfs(path.value().c_str(), &stats)) != 0) return false;
...
bool GetDiskSpaceInfo(const base::FilePath& path, int64_t* available_bytes,
  ...
  if (HANDLE_EINTR(statvfs(path.value().c_str(), &stats)) != 0) return false;
...
int SysInfo::NumberOfProcessors() {
  static base::NoDestructor<NumberOfProcessorsGetter> getter;
  return getter->value;
}
#endif  // !defined(OS_OPENBSD)
...
int64_t SysInfo::AmountOfVirtualMemory() {
  static base::NoDestructor<AmountOfVirtualMemoryGetter> getter;
  return getter->value;
}
...
// static
size_t SysInfo::VMAllocationGranularity() { return getpagesize(); }

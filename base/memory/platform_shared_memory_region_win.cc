...
size_t GetMemorySectionSize(void* address) {
  ...
  if (!::VirtualQuery(address, &memory_info, sizeof(memory_info))) return 0;
...
bool IsSectionSafeToMap(HANDLE handle) {
  static NtQuerySectionType nt_query_section_func =
      reinterpret_cast<NtQuerySectionType>(
          ::GetProcAddress(::GetModuleHandle("ntdll.dll"), "NtQuerySection"));
...
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Take(
    win::ScopedHandle handle, Mode mode, size_t size,
  ...
  if (!IsSectionSafeToMap(handle.Get())) return {};
...
bool PlatformSharedMemoryRegion::IsValid() const { return handle_.IsValid(); }
...
bool PlatformSharedMemoryRegion::MapAtInternal(off_t offset, size_t size,
                                               void** memory,

...
class BASE_EXPORT SharedMemoryMapping {
 ...
 protected:
  SharedMemoryMapping(void* address, size_t size, size_t mapped_size,
...
class BASE_EXPORT ReadOnlySharedMemoryMapping : public SharedMemoryMapping {
 public:
  ...
  // page-aligned.
  template <typename T>
  absl::Span<const T> GetMemoryAsSpan() const {
    static_assert(std::is_trivially_copyable<T>::value,
                  "Copying non-trivially-copyable object across memory spaces "
                  "is dangerous");
    if (!IsValid()) return absl::Span<const T>();
  ...
  template <typename T>
  absl::Span<const T> GetMemoryAsSpan(size_t count) const {
    static_assert(std::is_trivially_copyable<T>::value,
                  "Copying non-trivially-copyable object across memory spaces "
                  "is dangerous");
    if (!IsValid()) return absl::Span<const T>();
    if (size() / sizeof(T) < count) return absl::Span<const T>();
    return absl::Span<const T>(static_cast<const T*>(raw_memory_ptr()), count);
  }

 private:
  friend class ReadOnlySharedMemoryRegion;
  ReadOnlySharedMemoryMapping(void* address, size_t size, size_t mapped_size,
                              const UnguessableToken& guid);
};
...
class BASE_EXPORT WritableSharedMemoryMapping : public SharedMemoryMapping {
 public:
  ...
  template <typename T>
  absl::Span<T> GetMemoryAsSpan() const {
    static_assert(std::is_trivially_copyable<T>::value,
                  "Copying non-trivially-copyable object across memory spaces "
                  "is dangerous");
    if (!IsValid()) return absl::Span<T>();
    size_t count = size() / sizeof(T);
    return GetMemoryAsSpan<T>(count);
  }
  ...
  template <typename T>
  absl::Span<T> GetMemoryAsSpan(size_t count) const {
    static_assert(std::is_trivially_copyable<T>::value,
                  "Copying non-trivially-copyable object across memory spaces "
                  "is dangerous");
    if (!IsValid()) return absl::Span<T>();
    if (size() / sizeof(T) < count) return absl::Span<T>();
    return absl::Span<T>(static_cast<T*>(raw_memory_ptr()), count);
  }

 private:
  friend WritableSharedMemoryMapping MapAtForTesting(
      subtle::PlatformSharedMemoryRegion* region, off_t offset, size_t size);
  friend class ReadOnlySharedMemoryRegion;
  friend class WritableSharedMemoryRegion;
  friend class UnsafeSharedMemoryRegion;
  WritableSharedMemoryMapping(void* address, size_t size, size_t mapped_size,
                              const UnguessableToken& guid);
};

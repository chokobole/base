...
struct ScopedPathUnlinkerTraits {
  ...
  static void Free(const FilePath* path) {
    if (unlink(path->value().c_str())) PLOG(WARNING) << "unlink";
  }
};
...
FDPair ScopedFDPair::get() const { return {fd.get(), readonly_fd.get()}; }

// static
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Take(
    ScopedFDPair handle, Mode mode, size_t size, const UnguessableToken& guid) {
  if (!handle.fd.is_valid()) return {};
...
// static
PlatformSharedMemoryRegion PlatformSharedMemoryRegion::Take(
    ScopedFD handle, Mode mode, size_t size, const UnguessableToken& guid) {

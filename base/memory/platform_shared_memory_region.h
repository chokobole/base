...
// Platform-specific shared memory type used by this class.
  ...
  // are invalid.
  static PlatformSharedMemoryRegion Take(ScopedPlatformHandle handle, Mode mode,
                                         size_t size,
  ...
  static PlatformSharedMemoryRegion Take(ScopedFD handle, Mode mode,
                                         size_t size,
                                         const UnguessableToken& guid);
#endif
  ...
  bool MapAt(off_t offset, size_t size, void** memory,
 ...
 private:
  ...
  static bool CheckPlatformHandlePermissionsCorrespondToMode(
      PlatformHandle handle, Mode mode, size_t size);

  PlatformSharedMemoryRegion(ScopedPlatformHandle handle, Mode mode,
                             size_t size, const UnguessableToken& guid);

  bool MapAtInternal(off_t offset, size_t size, void** memory,
                     size_t* mapped_size) const;

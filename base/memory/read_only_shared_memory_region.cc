...
MappedReadOnlyRegion ReadOnlySharedMemoryRegion::Create(size_t size) {
  if (create_hook_) return create_hook_(size);
  ...
  if (!handle.MapAt(0, handle.GetSize(), &memory_ptr, &mapped_size)) return {};
  ...
  if (!region.IsValid() || !mapping.IsValid()) return {};
...
ReadOnlySharedMemoryMapping ReadOnlySharedMemoryRegion::MapAt(
  ...
  void* memory = nullptr;
  size_t mapped_size = 0;
  if (!handle_.MapAt(offset, size, &memory, &mapped_size)) return {};

  return ReadOnlySharedMemoryMapping(memory, size, mapped_size,
                                     handle_.GetGUID());
}

bool ReadOnlySharedMemoryRegion::IsValid() const { return handle_.IsValid(); }

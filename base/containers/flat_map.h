...
class flat_map
    : public ::base::internal::flat_tree<
          Key, std::pair<Key, Mapped>,
          ::base::internal::GetKeyFromValuePairFirst<Key, Mapped>, Compare> {
 private:
 ...
 public:
  ...
  flat_map(std::vector<value_type> items, const Compare& comp = Compare());
...
auto flat_map<Key, Mapped, Compare>::insert_or_assign(K&& key, M&& obj)
  ...
  if (!result.second) result.first->second = std::forward<M>(obj);
  return result;
}
...
template <class Key, class Mapped, class Compare>
template <class K, class... Args>
auto flat_map<Key, Mapped, Compare>::try_emplace(const_iterator hint, K&& key,
                                                 Args&&... args)
    -> std::enable_if_t<std::is_constructible<key_type, K&&>::value, iterator> {

...
class BASE_EXPORT MachLogMessage : public google::LogMessage {
 public:
  MachLogMessage(const char* file_path, int line, google::LogSeverity severity,
                 mach_error_t mach_err);
  MachLogMessage(const MachLogMessage& other) = delete;
  MachLogMessage& operator=(const MachLogMessage& other) = delete;
  ~MachLogMessage();
...
#define MACH_LOG_STREAM(severity, mach_err)                                \
  ::logging::MachLogMessage(__FILE__, __LINE__, ::google::GLOG_##severity, \
                            mach_err)                                      \
      .stream()
#define MACH_VLOG_STREAM(verbose_level, mach_err)                         \
  ::logging::MachLogMessage(__FILE__, __LINE__, -verbose_level, mach_err) \
      .stream()

#define MACH_LOG(severity, mach_err) \
  LAZY_STREAM(MACH_LOG_STREAM(severity, mach_err), true)
#define MACH_LOG_IF(severity, condition, mach_err) \
  LAZY_STREAM(MACH_LOG_STREAM(severity, mach_err), (condition))
...
#define MACH_VLOG_IF(verbose_level, condition, mach_err) \
  LAZY_STREAM(MACH_VLOG_STREAM(verbose_level, mach_err), (condition))
...
class BASE_EXPORT BootstrapLogMessage : public google::LogMessage {
 public:
  BootstrapLogMessage(const char* file_path, int line,
                      google::LogSeverity severity,
                      kern_return_t bootstrap_err);
  BootstrapLogMessage(const BootstrapLogMessage& other) = delete;
  BootstrapLogMessage& operator=(const BootstrapLogMessage& other) = delete;
  ~BootstrapLogMessage();
...
#define BOOTSTRAP_LOG_STREAM(severity, bootstrap_err) \
  ::logging::BootstrapLogMessage(__FILE__, __LINE__,  \
                                 ::google::GLOG_##severity, bootstrap_err)
...
#define BOOTSTRAP_LOG(severity, bootstrap_err) \
  LAZY_STREAM(BOOTSTRAP_LOG_STREAM(severity, bootstrap_err), true)
#define BOOTSTRAP_LOG_IF(severity, condition, bootstrap_err) \
  LAZY_STREAM(BOOTSTRAP_LOG_STREAM(severity, bootstrap_err), (condition))

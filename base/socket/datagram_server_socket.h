...
#ifndef BASE_SOCKET_DATAGRAM_SERVER_SOCKET_H_
#define BASE_SOCKET_DATAGRAM_SERVER_SOCKET_H_
...
// A UDP Socket.
class BASE_EXPORT DatagramServerSocket : public DatagramSocket {
 public:
  ...
  virtual int RecvFrom(std::shared_ptr<IOBuffer> buf, int buf_len,
                       IPEndPoint* address,
  ...
  // If ERR_IO_PENDING is returned, the caller must keep |buf| and |address|
  // alive until the callback is called.
  virtual int SendTo(std::shared_ptr<IOBuffer> buf, int buf_len,
...
#endif  // BASE_SOCKET_DATAGRAM_SERVER_SOCKET_H_

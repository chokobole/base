...
#ifndef BASE_SOCKET_UDP_SOCKET_WIN_H_
#define BASE_SOCKET_UDP_SOCKET_WIN_H_
...
class BASE_EXPORT QwaveApi {
 ...
 public:
  QwaveApi();
  QwaveApi(const QwaveApi& other) = delete;
  QwaveApi& operator=(const QwaveApi& other) = delete;
...
class BASE_EXPORT DscpManager {
 public:
  DscpManager(QwaveApi* api, SOCKET socket);
  DscpManager(const DscpManager& other) = delete;
  DscpManager& operator=(const DscpManager& other) = delete;
  ~DscpManager();
 ...
 private:
  ...
  static void OnHandleCreated(QwaveApi* api,
                              std::weak_ptr<DscpManager> dscp_manager,
                              HANDLE handle);
...
class BASE_EXPORT UDPSocketWin : public win::ObjectWatcher::Delegate {
 public:
  ...
  int RecvFrom(std::shared_ptr<IOBuffer> buf,
               int buf_len,
               IPEndPoint* address,
               CompletionOnceCallback callback);
  ...
  // data is sent.
  int SendTo(std::shared_ptr<IOBuffer> buf,
             int buf_len,
 ...
 private:
  ...
  // set to NULL.
  int SendToOrWrite(std::shared_ptr<IOBuffer> buf,
                    int buf_len,
                    const IPEndPoint* address,
                    CompletionOnceCallback callback);
  ...
  win::ObjectWatcher read_write_watcher_;

  // Events for read and write.
  win::ScopedHandle read_write_event_;
...
#endif  // BASE_SOCKET_UDP_SOCKET_WIN_H_

...
// Modified from net/base/net_errors_posix.cc
...
SocketErrorCode MapSystemError(int os_error) {
  if (os_error != 0) DVLOG(2) << "Error " << os_error;
  ...
  switch (os_error) {
    ...
    case 0:
      return OK;
    default:
      LOG(WARNING) << "Unknown error " << google::StrError(os_error) << " ("

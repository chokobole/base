...
#ifndef BASE_SOCKET_IP_ADDRESS_H_
#define BASE_SOCKET_IP_ADDRESS_H_
...
class BASE_EXPORT IPAddressBytes {
 public:
  IPAddressBytes();
...
class BASE_EXPORT IPAddress {
 public:
  ...
  bool AssignFromIPLiteral(absl::string_view ip_literal) WARN_UNUSED_RESULT;
...
BASE_EXPORT std::string IPAddressToStringWithPort(const IPAddress& address,
                                                  uint16_t port);

// Returns the address as a sequence of bytes in network-byte-order.
BASE_EXPORT std::string IPAddressToPackedString(const IPAddress& address);

// Converts an IPv4 address to an IPv4-mapped IPv6 address.
// For example 192.168.0.1 would be converted to ::ffff:192.168.0.1.
BASE_EXPORT IPAddress ConvertIPv4ToIPv4MappedIPv6(const IPAddress& address);

// Converts an IPv4-mapped IPv6 address to IPv4 address. Should only be called
// on IPv4-mapped IPv6 addresses.
BASE_EXPORT IPAddress ConvertIPv4MappedIPv6ToIPv4(const IPAddress& address);
...
// (IPv6) addresses.
BASE_EXPORT bool IPAddressMatchesPrefix(const IPAddress& ip_address,
...
#endif  // BASE_SOCKET_IP_ADDRESS_H_

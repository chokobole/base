...
class UDPSocketWin::Core {
 public:
  ...
  std::shared_ptr<IOBuffer> read_iobuffer_;
  std::shared_ptr<IOBuffer> write_iobuffer_;

  // The address storage passed to WSARecvFrom().
  SockaddrStorage recv_addr_storage_;

 private:
  class ReadDelegate : public win::ObjectWatcher::Delegate {
   public:
  ...
  class WriteDelegate : public win::ObjectWatcher::Delegate {
   public:
  ...
  win::ObjectWatcher read_watcher_;
  // |write_watcher_| watches for events from Write();
  win::ObjectWatcher write_watcher_;
};
...
QwaveApi* QwaveApi::GetDefault() {
  static NoDestructor<QwaveApi> qwave_api;
  return qwave_api.get();
}
...
int UDPSocketWin::Read(std::shared_ptr<IOBuffer> buf,
                       int buf_len,
                       CompletionOnceCallback callback) {
  return RecvFrom(buf, buf_len, nullptr, std::move(callback));
}

int UDPSocketWin::RecvFrom(std::shared_ptr<IOBuffer> buf,
                           int buf_len,
                           IPEndPoint* address,
                           CompletionOnceCallback callback) {
...
int UDPSocketWin::SendTo(std::shared_ptr<IOBuffer> buf,
                         int buf_len,
...
int UDPSocketWin::SendToOrWrite(std::shared_ptr<IOBuffer> buf,
                                int buf_len,
                                const IPEndPoint* address,
                                CompletionOnceCallback callback) {
  DCHECK_NE(INVALID_SOCKET, socket_);
...
void UDPSocketWin::OnObjectSignaled(HANDLE object) {
  ...
  WeakPtr<UDPSocketWin> event_pending = event_pending_.GetWeakPtr();
  if (rv == SOCKET_ERROR) {
...
void DscpManager::RequestHandle() {
  ...
  PostTaskAndReplyWithResult(
      FROM_HERE, {ThreadPool(), MayBlock()},
      BindOnce(&DscpManager::DoCreateHandle, api_),
      BindOnce(&DscpManager::OnHandleCreated, api_,
...
void DscpManager::OnHandleCreated(QwaveApi* api,
                                  WeakPtr<DscpManager> dscp_manager,
                                  HANDLE handle) {
  if (!handle)
    api->OnFatalError();

...
UnixDomainClientSocket::~UnixDomainClientSocket() { Disconnect(); }
...
void UnixDomainClientSocket::Disconnect() { socket_.reset(); }
...
int UnixDomainClientSocket::Read(std::shared_ptr<IOBuffer> buf, int buf_len,
                                 CompletionOnceCallback callback) {
  DCHECK(socket_);
  return socket_->Read(buf, buf_len, std::move(callback));
}

int UnixDomainClientSocket::Write(std::shared_ptr<IOBuffer> buf, int buf_len,
                                  CompletionOnceCallback callback) {
  DCHECK(socket_);

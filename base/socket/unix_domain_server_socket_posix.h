...
#ifndef BASE_SOCKET_UNIX_DOMAIN_SERVER_SOCKET_POSIX_H_
#define BASE_SOCKET_UNIX_DOMAIN_SERVER_SOCKET_POSIX_H_
...
class BASE_EXPORT UnixDomainServerSocket : public ServerSocket {
 public:
  ...
  using AuthCallback = RepeatingCallback<bool(const Credentials&)>;
  ...
  int ListenWithAddressAndPort(const std::string& address_string, uint16_t port,
 ...
 private:
  ...
  typedef RepeatingCallback<void(std::unique_ptr<SocketPosix>)> SetterCallback;

  int DoAccept(const SetterCallback& setter_callback);
  void AcceptCompleted(const SetterCallback& setter_callback, int rv);
...
#endif  // BASE_SOCKET_UNIX_DOMAIN_SERVER_SOCKET_POSIX_H_

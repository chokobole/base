...
int TCPSocketWin::Read(std::shared_ptr<IOBuffer> buf,
                       int buf_len,
                       CompletionOnceCallback callback) {
  DCHECK(!core_->read_iobuffer_.get());
  int rv =
      ReadIfReady(buf, buf_len,
                  Bind(&TCPSocketWin::RetryRead, Unretained(this)));
  if (rv != ERR_IO_PENDING)
    return rv;
  read_callback_ = std::move(callback);
...
int TCPSocketWin::ReadIfReady(std::shared_ptr<IOBuffer> buf,
                              int buf_len,
                              CompletionOnceCallback callback) {
  DCHECK_NE(socket_, INVALID_SOCKET);
...
bool TCPSocketWin::GetEstimatedRoundTripTime(TimeDelta* out_rtt) const {
  DCHECK(out_rtt);

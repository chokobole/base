...
const absl::Duration kWriteAsyncMsThreshold = absl::Milliseconds(1);
...
int GetIPv4AddressFromIndex(int socket, uint32_t index, uint32_t* address) {
  ...
  if (!if_indextoname(index, ifr.ifr_name)) return MapSystemError(errno);
  int rv = ioctl(socket, SIOCGIFADDR, &ifr);
  if (rv == -1) return MapSystemError(errno);
  result = reinterpret_cast<sockaddr_in*>(&ifr.ifr_addr);

  if (!result) return ERR_ADDRESS_INVALID;
...
typedef int (*ChangeFdguardNpFunction)(int fd, const guardid_t* guard,
                                       u_int flags, const guardid_t* nguard,
                                       u_int nflags, int* fdflagsp);
...
int GetSocketFDHash(int fd) { return fd ^ 1595649551; }

}  // namespace

UDPSocketPosix::Sender::Sender() : sendmmsg_enabled_(false) {}
UDPSocketPosix::Sender::~Sender() {}

UDPSocketPosix::SendResult::SendResult() : rv(0), write_count(0) {}
UDPSocketPosix::SendResult::~SendResult() {}
UDPSocketPosix::SendResult::SendResult(int _rv, int _write_count,
                                       DatagramBuffers _buffers)
    : rv(_rv), write_count(_write_count), buffers(std::move(_buffers)) {}
UDPSocketPosix::SendResult::SendResult(SendResult&& other) = default;

UDPSocketPosix::SendResult UDPSocketPosix::Sender::InternalSendBuffers(
    int fd, DatagramBuffers buffers) const {
  int rv = 0;
  int write_count = 0;
...
#if HAVE_SENDMMSG
UDPSocketPosix::SendResult UDPSocketPosix::Sender::InternalSendmmsgBuffers(
    int fd, DatagramBuffers buffers) const {
  StackVector<struct iovec, kWriteAsyncMaxBuffersThreshold + 1> msg_iov;
  StackVector<struct mmsghdr, kWriteAsyncMaxBuffersThreshold + 1> msgvec;
...
UDPSocketPosix::SendResult UDPSocketPosix::Sender::SendBuffers(
    int fd, DatagramBuffers buffers) {
#if HAVE_SENDMMSG
...
ssize_t UDPSocketPosix::Sender::Send(int sockfd, const void* buf, size_t len,
                                     int flags) const {
  return send(sockfd, buf, len, flags);
}

#if HAVE_SENDMMSG
int UDPSocketPosix::Sender::Sendmmsg(int sockfd, struct mmsghdr* msgvec,
                                     unsigned int vlen,
...
UDPSocketPosix::~UDPSocketPosix() { Close(); }
...
int UDPSocketPosix::Open(AddressFamily address_family) {
  ...
  if (socket_ == kInvalidSocket) return MapSystemError(errno);
#if defined(OS_MACOSX) && !defined(OS_IOS)
...
int UDPSocketPosix::GetPeerAddress(IPEndPoint* address) const {
  DCHECK(address);
  if (!is_connected()) return ERR_SOCKET_NOT_CONNECTED;
...
int UDPSocketPosix::Read(std::shared_ptr<IOBuffer> buf, int buf_len,
                         CompletionOnceCallback callback) {
  return RecvFrom(buf, buf_len, nullptr, std::move(callback));
}

int UDPSocketPosix::RecvFrom(std::shared_ptr<IOBuffer> buf, int buf_len,
                             IPEndPoint* address,
                             CompletionOnceCallback callback) {
  ...
  int nread = InternalRecvFrom(buf.get(), buf_len, address);
  if (nread != ERR_IO_PENDING) return nread;

  if (!EventLoop::Current()->WatchFileDescriptor(
          socket_, true, EventLoop::WATCH_READ, &read_socket_watcher_,
          &read_watcher_)) {
...
int UDPSocketPosix::Write(std::shared_ptr<IOBuffer> buf, int buf_len,
                          CompletionOnceCallback callback) {
  return SendToOrWrite(buf, buf_len, nullptr, std::move(callback));
}

int UDPSocketPosix::SendTo(std::shared_ptr<IOBuffer> buf, int buf_len,
                           const IPEndPoint& address,
                           CompletionOnceCallback callback) {
  return SendToOrWrite(buf, buf_len, &address, std::move(callback));
}

int UDPSocketPosix::SendToOrWrite(std::shared_ptr<IOBuffer> buf, int buf_len,
                                  const IPEndPoint* address,
                                  CompletionOnceCallback callback) {
  DCHECK_NE(kInvalidSocket, socket_);
  ...
  if (result != ERR_IO_PENDING) return result;
...
int UDPSocketPosix::InternalConnect(const IPEndPoint& address) {
  ...
  if (rv < 0) return MapSystemError(errno);
...
int UDPSocketPosix::AllowAddressSharingForMulticast() {
  ...
  if (rv != 0 && errno != ENOPROTOOPT) return MapSystemError(errno);
#endif  // SO_REUSEPORT

  return OK;
}

void UDPSocketPosix::ReadWatcher::OnFileCanRead(int) {
  if (!socket_->read_callback_.is_null()) socket_->DidCompleteRead();
}

void UDPSocketPosix::WriteWatcher::OnFileCanWrite(int) {
  if (!socket_->write_callback_.is_null()) socket_->DidCompleteWrite();
}
...
int UDPSocketPosix::InternalRecvFrom(IOBuffer* buf, int buf_len,
                                     IPEndPoint* address) {
...
int UDPSocketPosix::InternalRecvFromConnectedSocket(IOBuffer* buf, int buf_len,
                                                    IPEndPoint* address) {
  ...
  if (bytes_transferred < 0) {
    ...
    if (address) *address = *remote_address_.get();
  }
...
int UDPSocketPosix::InternalSendTo(std::shared_ptr<IOBuffer> buf, int buf_len,
                                   const IPEndPoint* address) {
  SockaddrStorage storage;
  ...
  if (result < 0) result = MapSystemError(errno);
  return result;
}
  ...
  if (multicast_interface_ != 0) {
    switch (addr_family_) {
      case AF_INET: {
        ...
        if (rv) return MapSystemError(errno);
        break;
      }
      case AF_INET6: {
...
#if defined(OS_MACOSX)
  if (last_error == EADDRNOTAVAIL) return ERR_ADDRESS_IN_USE;
#endif
...
int UDPSocketPosix::RandomBind(const IPAddress& address) {
  ...
  for (int i = 0; i < kBindRetries; ++i) {
    int rv = DoBind(
        IPEndPoint(address, absl::Uniform(bitgen, kPortStart, kPortEnd)));
    if (rv != ERR_ADDRESS_IN_USE) return rv;
  }
...
int UDPSocketPosix::JoinGroup(const IPAddress& group_address) const {
  ...
  switch (group_address.size()) {
    case IPAddress::kIPv4AddressSize: {
      if (addr_family_ != AF_INET) return ERR_ADDRESS_INVALID;
...
int UDPSocketPosix::SetMulticastInterface(uint32_t interface_index) {
  if (is_connected()) return ERR_SOCKET_IS_CONNECTED;
...
int UDPSocketPosix::SetMulticastTimeToLive(int time_to_live) {
  ...
  if (time_to_live < 0 || time_to_live > 255) return ERR_INVALID_ARGUMENT;
...
int UDPSocketPosix::WriteAsync(const char* buffer, size_t buf_len,
                               CompletionOnceCallback callback) {
  ...
  return InternalWriteAsync(std::move(callback));
}

int UDPSocketPosix::WriteAsync(DatagramBuffers buffers,
                               CompletionOnceCallback callback) {
...
int UDPSocketPosix::InternalWriteAsync(CompletionOnceCallback callback) {
  CHECK(write_callback_.is_null());
...
void UDPSocketPosix::FlushPending() {
  ...
  if (write_async_watcher_->watching()) return;

  if (pending_writes_.empty()) return;

  // if (write_async_timer_running_) write_async_timer_.Reset();
...
void UDPSocketPosix::DidSendBuffers(SendResult send_result) {
  ...
  if (write_callback_.is_null()) return;
...
void UDPSocketPosix::WriteAsyncWatcher::OnFileCanWrite(int) {
...
bool UDPSocketPosix::WatchFileDescriptor() {
  if (write_async_watcher_->watching()) return true;
...
bool UDPSocketPosix::InternalWatchFileDescriptor() {
  return EventLoop::Current()->WatchFileDescriptor(
      socket_, true, EventLoop::WATCH_WRITE, &write_socket_watcher_,
      write_async_watcher_.get());
}

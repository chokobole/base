...
#include "base/socket/udp_server_socket.h"
...
int UDPServerSocket::RecvFrom(std::shared_ptr<IOBuffer> buf, int buf_len,
                              IPEndPoint* address,
                              CompletionOnceCallback callback) {
  return socket_.RecvFrom(buf, buf_len, address, std::move(callback));
}

int UDPServerSocket::SendTo(std::shared_ptr<IOBuffer> buf, int buf_len,
...
int UDPServerSocket::SetDoNotFragment() { return socket_.SetDoNotFragment(); }

void UDPServerSocket::SetMsgConfirm(bool confirm) {
  return socket_.SetMsgConfirm(confirm);
}

void UDPServerSocket::Close() { socket_.Close(); }
...
void UDPServerSocket::AllowAddressReuse() { allow_address_reuse_ = true; }

void UDPServerSocket::AllowBroadcast() { allow_broadcast_ = true; }

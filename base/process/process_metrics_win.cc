...
absl::Duration ProcessMetrics::GetCumulativeCPUUsage() {
  ...
  if (!process_.IsValid()) return absl::ZeroDuration();
  ...
  return absl::time_internal::ToUnixDuration(FromFileTime(kernel_time)) +
         absl::time_internal::ToUnixDuration(FromFileTime(user_time));
}
...
size_t GetPageSize() { return PAGESIZE_KB * 1024; }
...
bool GetSystemMemoryInfo(SystemMemoryInfoKB* meminfo) {
  ...
  if (!::GlobalMemoryStatusEx(&mem_status)) return false;
...
BASE_EXPORT bool GetSystemPerformanceInfo(SystemPerformanceInfo* info) {
  ...
  if (!query_system_information_ptr) return false;

...
void TrimKeyValuePairs(StringPairs* pairs) {
  for (auto& pair : *pairs) {
    absl::StripAsciiWhitespace(&pair.first);
    absl::StripAsciiWhitespace(&pair.second);
  }
}
...
uint64_t ReadFileToUint64(const FilePath& file) {
  ...
  if (!ReadFileToString(file, &file_contents)) return 0;
  absl::StripAsciiWhitespace(&file_contents);
  uint64_t file_contents_uint64 = 0;
  if (!StringToUint64(file_contents, &file_contents_uint64)) return 0;
...
bool ReadProcFileToTrimmedStringPairs(pid_t pid, absl::string_view filename,
                                      StringPairs* key_value_pairs) {
  std::string status_data;
  FilePath status_file = internal::GetProcPidDir(pid).Append(filename);
  if (!ReadFileToString(status_file, &status_data)) return false;
...
size_t ReadProcStatusAndGetFieldAsSizeT(pid_t pid, absl::string_view field) {
  StringPairs pairs;
  if (!ReadProcFileToTrimmedStringPairs(pid, "status", &pairs)) return 0;
  ...
  for (const auto& pair : pairs) {
    ...
    std::vector<absl::string_view> split_value_str =
        SplitStringView(value_str, " ", TRIM_WHITESPACE, SPLIT_WANT_ALL);
...
bool ReadProcStatusAndGetFieldAsUint64(pid_t pid, absl::string_view field,
                                       uint64_t* result) {
  StringPairs pairs;
  if (!ReadProcFileToTrimmedStringPairs(pid, "status", &pairs)) return false;
  ...
  for (const auto& pair : pairs) {
    ...
    uint64_t value;
    if (!StringToUint64(value_str, &value)) return false;
    *result = value;
    return true;
  }
  return false;
...
void ReadChromeOSGraphicsMemory(SystemMemoryInfoKB* meminfo) {
  ...
  if (ReadFileToString(mali_memory_file, &mali_memory_data)) {
    ...
    if (num_res == 1) meminfo->gem_size += mali_size;
  }
...
absl::Duration ProcessMetrics::GetCumulativeCPUUsage() {
  return internal::ClockTicksToDuration(GetProcessCPU(process_));
}
...
bool ProcessMetrics::GetIOCounters(IoCounters* io_counters) const {
  StringPairs pairs;
  if (!ReadProcFileToTrimmedStringPairs(process_, "io", &pairs)) return false;
...
bool ProcessMetrics::GetPageFaultCounts(PageFaultCounts* counts) const {
  ...
  if (!internal::ReadProcStats(process_, &stats_data)) return false;
  std::vector<std::string> proc_stats;
  if (!internal::ParseProcStats(stats_data, &proc_stats)) return false;
...
int ProcessMetrics::GetOpenFdCount() const {
  ...
  if (!dir_reader.IsValid()) return -1;

  int total_count = 0;
  for (; dir_reader.Next();) {
    const char* name = dir_reader.name();
    if (strcmp(name, ".") != 0 && strcmp(name, "..") != 0) ++total_count;
  }
...
int ProcessMetrics::GetOpenFdSoftLimit() const {
  ...
  if (!ReadFileToString(fd_path, &limits_contents)) return -1;

  for (const auto& line : SplitStringView(
           limits_contents, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
    if (!base::StartsWith(line, "Max open files")) continue;

    auto tokens =
        SplitStringView(line, " ", TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);
    if (tokens.size() > 3) {
      int limit = -1;
      if (!StringToInt(tokens[3], &limit)) return -1;
      return limit;
    }
  }
  return -1;
...
int ParseProcStatCPU(absl::string_view input) {
  ...
  while ((i = input.find(' ', i + 1)) != input.npos) {
    ...
    if (--num_spaces_remaining == 0) {
      int utime = 0;
      int stime = 0;
      if (sscanf(&input.data()[i], "%d %d", &utime, &stime) != 2) return -1;
...
bool ParseProcMeminfo(absl::string_view meminfo_data,
                      SystemMemoryInfoKB* meminfo) {
  ...
  for (absl::string_view line : SplitStringView(
           meminfo_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY)) {
    std::vector<absl::string_view> tokens = SplitStringView(
    ...
    // usually video memory otherwise invisible to the OS.
      ...
      target = &meminfo->slab;
#endif
    if (target) StringToInt(tokens[1], target);
  }
...
bool ParseProcVmstat(absl::string_view vmstat_data, VmStatInfo* vmstat) {
  ...
  for (absl::string_view line : SplitStringView(
    ...
    std::vector<absl::string_view> tokens =
        SplitStringView(line, " ", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
    if (tokens.size() != 2) continue;

    uint64_t val;
    if (!StringToUint64(tokens[1], &val)) continue;
    ...
    if (has_pswpin && has_pswpout && has_pgmajfault) return true;
  }
...
bool IsValidDiskName(absl::string_view candidate) {
  ...
  if (candidate[1] == 'd' &&
    ...
    for (size_t i = 2; i < candidate.length(); ++i) {
      if (!islower(candidate[i])) return false;
    }
    return true;
  }

  const char kMMCName[] = "mmcblk";
  if (!base::StartsWith(candidate, kMMCName)) return false;
...
bool GetSystemDiskInfo(SystemDiskInfo* diskinfo) {
  ...
  std::vector<absl::string_view> diskinfo_lines = SplitStringView(
      diskinfo_data, "\n", KEEP_WHITESPACE, SPLIT_WANT_NONEMPTY);
  ...
  for (absl::string_view line : diskinfo_lines) {
    std::vector<absl::string_view> disk_fields = SplitStringView(
        line, kWhitespaceASCII, TRIM_WHITESPACE, SPLIT_WANT_NONEMPTY);

    // Fields may have overflowed and reset to zero.
    if (!IsValidDiskName(std::string(disk_fields[kDiskDriveName]))) continue;
...
absl::Duration GetUserCpuTimeSinceBoot() {
  return internal::GetUserCpuTimeSinceBoot();
}

#if defined(OS_CHROMEOS)
bool ParseZramMmStat(absl::string_view mm_stat_data, SwapInfo* swap_info) {
  ...
  if (!StringToUint64(tokens[0], &swap_info->orig_data_size)) return false;
  if (!StringToUint64(tokens[1], &swap_info->compr_data_size)) return false;
  if (!StringToUint64(tokens[2], &swap_info->mem_used_total)) return false;

  return true;
}

bool ParseZramStat(absl::string_view stat_data, SwapInfo* swap_info) {
  ...
  if (!StringToUint64(tokens[0], &swap_info->num_reads)) return false;
  if (!StringToUint64(tokens[4], &swap_info->num_writes)) return false;
...
void ParseZramPath(SwapInfo* swap_info) {
  ...
  if (IgnoreZramFirstPage(orig_data_size, swap_info)) return;
...
bool GetSwapInfoImpl(SwapInfo* swap_info) {
  ...
  if (IgnoreZramFirstPage(swap_info->orig_data_size, swap_info)) return true;

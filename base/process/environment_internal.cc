...
size_t ParseEnvLine(const char* input, std::string* key) {
  // Skip to the equals or end of the string, this is the key.
  size_t cur = 0;
  while (input[cur] && input[cur] != '=') cur++;
  *key = std::string(&input[0], cur);
...
std::string AlterEnvironment(const char* env, const EnvironmentMap& changes) {
  std::string result;
  ...
  for (const auto& i : changes) {
    ...
    CHECK_EQ(std::string::npos, i.first.find('\0'));
    CHECK_EQ(std::string::npos, i.second.find('\0'));
    if (!i.second.empty()) {
      result += i.first;
      result.push_back('=');
      result += i.second;

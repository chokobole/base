...
int pm_sample_task(mach_port_t task, OpaquePMTaskEnergyData* pm_energy,
                   uint64_t mach_time, uint8_t flags);
...
std::unique_ptr<ProcessMetrics> ProcessMetrics::CreateProcessMetrics(
  ...
  return absl::WrapUnique(new ProcessMetrics(process, port_provider));
}
...
absl::Duration ProcessMetrics::GetCumulativeCPUUsage() {
  ...
  if (task == MACH_PORT_NULL) return absl::ZeroDuration();
  ...
  if (!GetTaskInfo(task, &task_info_data)) return absl::ZeroDuration();
  ...
  return absl::Microseconds(TimeValToMicroseconds(task_timeval));
}
...
int ProcessMetrics::GetEnergyImpact() {
  ...
  double seconds_since_last_measurement = absl::ToDoubleMicroseconds(
      absl::time_internal::ToUnixDuration(FromMachAbsoluteTime(delta)));
...
int ProcessMetrics::GetOpenFdSoftLimit() const { return GetMaxFds(); }
...
mach_port_t ProcessMetrics::TaskForPid(ProcessHandle process) const {
  ...
  if (port_provider_) task = port_provider_->TaskForPid(process_);
  if (task == MACH_PORT_NULL && process_ == getpid()) task = mach_task_self();
  return task;
}
...
MachVMRegionResult GetTopInfo(mach_port_t task, mach_vm_size_t* size,
...
MachVMRegionResult GetBasicInfo(mach_port_t task, mach_vm_size_t* size,
                                mach_vm_address_t* address,

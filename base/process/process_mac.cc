...
absl::Time Process::CreationTime() const {
  ...
  size_t len = 0;
  if (sysctl(mib, size(mib), NULL, &len, NULL, 0) < 0) return absl::Time();
  ...
  if (sysctl(mib, size(mib), proc.get(), &len, NULL, 0) < 0)
    return absl::Time();
  return absl::TimeFromTimeval(proc->kp_proc.p_un.__p_starttime);
}
...
bool Process::IsProcessBackgrounded(PortProvider* port_provider) const {
  DCHECK(IsValid());
  if (port_provider == nullptr || !CanBackgroundProcesses()) return false;

  mach_port_t task_port = port_provider->TaskForPid(Pid());
  if (task_port == TASK_NULL) return false;
...
bool Process::SetProcessBackgrounded(PortProvider* port_provider,
  ...
  if (IsProcessBackgrounded(port_provider) == background) return true;

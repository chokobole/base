...
// static
bool Process::CanBackgroundProcesses() { return true; }
...
bool Process::IsValid() const { return process_.IsValid() || is_current(); }
...
absl::Time Process::CreationTime() const {
  ...
  return FromFileTime(creation_time);
}

bool Process::is_current() const { return is_current_process_; }
...
bool Process::WaitForExit(int* exit_code) const {
  return WaitForExitWithTimeout(absl::Milliseconds(INFINITE), exit_code);
}

bool Process::WaitForExitWithTimeout(absl::Duration timeout,
                                     int* exit_code) const {
  // Limit timeout to INFINITE.
  DWORD timeout_ms = saturated_cast<DWORD>(absl::ToInt64Milliseconds(timeout));
  if (::WaitForSingleObject(Handle(), timeout_ms) != WAIT_OBJECT_0)
    return false;

  DWORD temp_code;  // Don't clobber out-parameters in case of failure.
  if (!::GetExitCodeProcess(Handle(), &temp_code)) return false;

  if (exit_code) *exit_code = temp_code;
...
bool Process::IsProcessBackgrounded() const {
  ...
  if (priority == 0) return false;  // Failure case.
...
bool Process::SetProcessBackgrounded(bool value) {
  ...
  if (is_current()) {
    priority =
        value ? PROCESS_MODE_BACKGROUND_BEGIN : PROCESS_MODE_BACKGROUND_END;
  } else {

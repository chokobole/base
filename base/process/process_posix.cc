...
bool WaitpidWithTimeout(base::ProcessHandle handle, int* status,
                        absl::Duration wait) {
  ...
  if (absl::time_internal::IsInfiniteDuration(wait)) {
  ...
  absl::Time wakeup_time = absl::Now() + wait;
  while (ret_pid == 0) {
    absl::Time now = absl::Now();
    if (now > wakeup_time) break;
    // Guaranteed to be non-negative!
    int64_t sleep_time_usecs = absl::ToInt64Microseconds(wakeup_time - now);
...
bool WaitForSingleNonChildProcess(base::ProcessHandle handle,
  ...
  // interrupted.
  bool wait_forever = absl::time_internal::IsInfiniteDuration(wait);
  absl::Duration remaining_delta;
  absl::Time deadline;
  if (!wait_forever) {
    remaining_delta = wait;
    deadline = absl::Now() + remaining_delta;
  }
  ...
  do {
    ...
    if (wait_forever) {
      ...
      remaining_timespec = absl::ToTimespec(remaining_delta);
    ...
    if (result == -1 && errno == EINTR) {
      if (!wait_forever) {
        remaining_delta = deadline - absl::Now();
      }
      result = 0;
    } else {
      break;
    }
  } while (wait_forever || remaining_delta > absl::ZeroDuration());
...
bool WaitForExitWithTimeoutImpl(base::ProcessHandle handle, int* exit_code,
  ...
  int status;
  if (!WaitpidWithTimeout(handle, &status, timeout)) return exited;
...
Process::Process(Process&& other) : process_(other.process_) { other.Close(); }
...
Process Process::Open(ProcessId pid) {
  if (pid == GetCurrentProcId()) return Current();
...
// static
bool Process::CanBackgroundProcesses() { return false; }
...
bool Process::IsValid() const { return process_ != kNullProcessHandle; }
...
bool Process::Terminate(int exit_code, bool wait) const {
  ...
  if (wait && did_terminate) {
    if (WaitForExitWithTimeout(absl::Seconds(60), nullptr)) return true;
    did_terminate = kill(process_, SIGKILL) == 0;
    if (did_terminate) return WaitForExit(nullptr);
  }
...
bool Process::WaitForExit(int* exit_code) const {
  return WaitForExitWithTimeout(absl::InfiniteDuration(), exit_code);
}

bool Process::WaitForExitWithTimeout(absl::Duration timeout,
                                     int* exit_code) const {
  int local_exit_code = 0;
  bool exited = WaitForExitWithTimeoutImpl(Handle(), &local_exit_code, timeout);
  if (exited) {
    if (exit_code) *exit_code = local_exit_code;
  }
  return exited;
}

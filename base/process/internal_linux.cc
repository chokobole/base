...
pid_t ProcDirSlotToPid(const char* d_name) {
  ...
  for (i = 0; i < NAME_MAX && d_name[i]; ++i) {
    if (!absl::ascii_isdigit(d_name[i])) {
      return 0;
    }
  }
  if (i == NAME_MAX) return 0;
...
bool ReadProcFile(const FilePath& file, std::string* buffer) {
  ...
  if (!ReadFileToString(file, buffer)) {
    DLOG(WARNING) << "Failed to read " << file.value();
    return false;
  }
  return !buffer->empty();
}
...
bool ParseProcStats(absl::string_view stats_data,
  ...
  proc_stats->clear();
  // PID.
  proc_stats->push_back(std::string(stats_data.substr(0, open_parens_idx)));
  // Process name without parentheses.
  proc_stats->push_back(std::string(stats_data.substr(
...
int64_t ReadStatFileAndGetFieldAsInt64(const FilePath& stat_file,
  ...
  if (!ReadProcFile(stat_file, &stats_data)) return 0;
  std::vector<std::string> proc_stats;
...
size_t ReadProcStatsAndGetFieldAsSizeT(pid_t pid, ProcStatsFields field_num) {
  std::string stats_data;
  if (!ReadProcStats(pid, &stats_data)) return 0;
  std::vector<std::string> proc_stats;
  if (!ParseProcStats(stats_data, &proc_stats)) return 0;
  return GetProcStatsFieldAsSizeT(proc_stats, field_num);
}

absl::Time GetBootTime() {
  FilePath path("/proc/stat");
  std::string contents;
  if (!ReadProcFile(path, &contents)) return absl::Time();
  ProcStatMap proc_stat;
  ParseProcStat(contents, &proc_stat);
  ProcStatMap::const_iterator btime_it = proc_stat.find("btime");
  if (btime_it == proc_stat.end()) return absl::Time();
  int btime;
  if (!StringToInt(btime_it->second, &btime)) return absl::Time();
  return absl::FromTimeT(btime);
}

absl::Duration GetUserCpuTimeSinceBoot() {
  FilePath path("/proc/stat");
  std::string contents;
  if (!ReadProcFile(path, &contents)) return absl::ZeroDuration();
  ...
  return ClockTicksToDuration(user + nice);
}

absl::Duration ClockTicksToDuration(int clock_ticks) {
  ...
  return absl::Seconds(static_cast<int64_t>(clock_ticks) / kHertz);
}

...
bool File::IsValid() const { return file_.IsValid(); }

PlatformFile File::GetPlatformFile() const { return file_.Get(); }

PlatformFile File::TakePlatformFile() { return file_.Take(); }
...
int64_t File::Seek(Whence whence, int64_t offset) {
  ...
  if (!SetFilePointerEx(file_.Get(), distance, &res, move_method)) return -1;
  return res.QuadPart;
}
...
int File::ReadAtCurrentPos(char* data, int size) {
  ...
  DWORD bytes_read;
  if (::ReadFile(file_.Get(), data, size, &bytes_read, NULL)) return bytes_read;
  if (ERROR_HANDLE_EOF == GetLastError()) return 0;
...
int64_t File::GetLength() {
  ...
  LARGE_INTEGER size;
  if (!::GetFileSizeEx(file_.Get(), &size)) return -1;
...
bool File::SetTimes(absl::Time last_access_time,
  ...
  FILETIME last_access_filetime = ToFileTime(last_access_time);
  FILETIME last_modified_filetime = ToFileTime(last_modified_time);
...
bool File::GetInfo(Info* info) {
  ...
  if (!GetFileInformationByHandle(file_.Get(), &file_info)) return false;
  ...
  info->last_modified = FromFileTime(file_info.ftLastWriteTime);
  info->last_accessed = FromFileTime(file_info.ftLastAccessTime);
  info->creation_time = FromFileTime(file_info.ftCreationTime);
  return true;
}
...
void File::SetPlatformFile(PlatformFile file) { file_.Set(file); }

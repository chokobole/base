...
absl::Time FileEnumerator::FileInfo::GetLastModifiedTime() const {
  return FromFileTime(find_data_.ftLastWriteTime);
}
...
FileEnumerator::FileEnumerator(const FilePath& root_path, bool recursive,
                               int file_type, const std::string& pattern)
...
FileEnumerator::FileEnumerator(const FilePath& root_path, bool recursive,
    ...
    : recursive_(recursive),
      ...
      pattern_(!pattern.empty() ? pattern : "*"),
...
FileEnumerator::~FileEnumerator() {
  if (find_handle_ != INVALID_HANDLE_VALUE) FindClose(find_handle_);
}
...
FilePath FileEnumerator::Next() {
  while (has_find_data_ || !pending_paths_.empty()) {
    ...
    if (IsTypeMatched(is_dir) && IsPatternMatched(filename)) return abs_path;
  }
  return FilePath();
}

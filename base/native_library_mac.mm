...
static NativeLibraryObjCStatus GetObjCStatusForImage(const void* function_pointer) {
  Dl_info info;
  if (!dladdr(function_pointer, &info)) return OBJC_UNKNOWN;
  ...
  const section_64* section = getsectbynamefromheader_64(
      reinterpret_cast<const struct mach_header_64*>(info.dli_fbase), SEG_DATA, "__objc_imageinfo");
...
NativeLibrary LoadNativeLibraryWithOptions(const FilePath& library_path,
  ...
  if (library_path.Extension() == "dylib" || !DirectoryExists(library_path)) {
    ...
    if (!dylib) {
      if (error) error->message = dlerror();
      return nullptr;
    }
  ...
  ScopedCFTypeRef<CFURLRef> url(CFURLCreateFromFileSystemRepresentation(
      ...
      library_path.value().length(), true));
...
void UnloadNativeLibrary(NativeLibrary library) {
  if (library->objc_status == OBJC_NOT_PRESENT) {
    if (library->type == BUNDLE) {
      CFBundleCloseBundleResourceMap(library->bundle, library->bundle_resource_ref);
...
void* GetFunctionPointerFromNativeLibrary(NativeLibrary library, absl::string_view name) {
  ...
  if (library->type == BUNDLE) {
    ScopedCFTypeRef<CFStringRef> symbol_name(
        CFStringCreateWithCString(kCFAllocatorDefault, name.data(), kCFStringEncodingUTF8));
    function_pointer = CFBundleGetFunctionPointerForName(library->bundle, symbol_name);
  } else {
...
std::string GetNativeLibraryName(absl::string_view name) {
  ...
  return absl::Substitute("lib$0.dylib", name);
}
